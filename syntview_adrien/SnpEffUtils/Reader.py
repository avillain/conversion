...
class SnpEffReader(object):
    ...
    def read_and_check(self, snp_fh):
		...
		chr,pos,_,ref,variant,qual,status,info,field,_=line.split("\t",10)
		...
		try:
			gene_name=re.search("SNPEFF_GENE_NAME=([^;]*);",info).groups()[0]
		except:
			try:
		...
		ref=variant
		variant=toto
            new_dict = {"position"  : string.atoi(pos),
                        "type"      : ty,
                        "reference" : ref,
                        "variant"   : variant,
                        "gene_name" : gene_name,
