...
if __name__ == '__main__':
	...
	ptt_list,names_list=ptt_reader.read_ptt(ptt_fh)
    ...
    with open(out_snp, "w") as out_snp_fh:
	    ...
	    print >>out_snp_fh, "%s, %s" %(out_snp, out_snp)
	    print >>out_indel_fh, "%s, %s" %(out_indel, out_indel)
	    bychr_sortedsnp_list=[sorted([snp for snp in snp_list if snp['chromosome']==chr], key=lambda k: k['position']) for chr in names_list] # list of lists of SNPs per chromosome sorted by position
	    for i in range(len(names_list)):
		print >>out_snp_fh, ">%s" %names_list[i]
		print >>out_snp_fh, "Position\tNucRef\tNucNR\tGene\tStrand\tAARef\tAANR\tReadsSNP\tTotReadsSNP"
		print >>out_indel_fh, ">%s" %names_list[i]
                print >>out_indel_fh, "Position\tNucRef\tNucNR\tGene\tStrand\tAARef\tAANR\tReadsSNP\tTotReadsSNP"
		for snp in bychr_sortedsnp_list[i]:
                    try:
                	type, reformatted_line = snp_reformatter.format(snp, ptt_list[i])
                	if type == "SNP":
